...
class Dataset:
    ...
    def batchs(self):
        ...
        for i in range(0, len(self.words) // BatchSize + 1):
            ...
            yield torch.LongTensor(self.words[L:R]), torch.LongTensor(self.pos[L:R]), torch.LongTensor(
                self.loc[L:R]), torch.FloatTensor(self.maskL[L:R]), torch.FloatTensor(
                self.maskR[L:R]), torch.LongTensor(self.label[L:R])
...
class uDataset:
    ...
    def batchs(self):
        ...
        for i in range(0, len(self.words) // BatchSize + 1):
            ...
            yield torch.LongTensor(bwords[nidx]), torch.LongTensor(bpos[nidx]), torch.LongTensor(
                bloc[nidx]), torch.FloatTensor(bmaskL[nidx]), torch.FloatTensor(bmaskR[nidx]), torch.LongTensor(
                blabel[nidx]), torch.LongTensor(bwords[uidx]), torch.LongTensor(bpos[uidx]), torch.LongTensor(
                bloc[uidx]), torch.FloatTensor(bmaskL[uidx]), torch.FloatTensor(bmaskR[uidx]), torch.LongTensor(
                blabel[uidx])
...
class joinDataset:
    ...
    def conf_batch(self):
        ...
        for i in range(0, self.it_times):
            ...
            yield torch.LongTensor(self.cwords[L:R]), torch.LongTensor(self.cpos[L:R]), torch.LongTensor(
                self.cloc[L:R]), torch.FloatTensor(self.cmaskL[L:R]), torch.FloatTensor(
                self.cmaskR[L:R]), torch.LongTensor(self.clabel[L:R])
    ...
    def unconf_batch(self):
        ...
        for i in range(0, self.it_times):
            ...
            yield torch.LongTensor(self.uwords[L:R]), torch.LongTensor(self.upos[L:R]), torch.LongTensor(
                self.uloc[L:R]), torch.FloatTensor(self.umaskL[L:R]), torch.FloatTensor(
                self.umaskR[L:R]), torch.LongTensor(self.ulabel[L:R]), self.utimes[L:R]
